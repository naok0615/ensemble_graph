...
class ClassificationTrainer():
    ...
    def top_att_id(self, attention):
        ...
        for data_id in range(len(attention)):
            ...
            top_id = attention[data_id].argmax().item()
            y_id = np.trunc(top_id/7)
            x_id = top_id - y_id*7
    ...
    def attention_masked(self, output, mask_per, **kwargs):
        for k in range(len(output)-1):  # モデル数 = ノード数 - 1(アンサンブルノード)
            #output[k+1][2] = [self.map2vec(output[k+1][2]), None]
