...
class AttLoss(_LossBase):
    ...
    def forward(self, target_output, source_output, label_id, log=None, **kwargs):
        ...
        # ------------------------------------------------------------------------------------------------
        # 11*11
        student_att_top_11 = torch.zeros(len(student_att), 11*11, device=student_att.device)
        teacher_att_top_11 = torch.zeros(len(teacher_att), 11*11, device=teacher_att.device)
        # 7*7
        student_att_top_7 = torch.zeros(len(student_att), 7*7, device=student_att.device)
        teacher_att_top_7 = torch.zeros(len(teacher_att), 7*7, device=teacher_att.device)
        # 3*3
        ...
        for i in range(len(student_att)):
            ...
            student_att_top_11[i] = torch.cat((student_att[i][ x_1+14*y_id[0] : x_2+14*y_id[0] ],
                                               student_att[i][ x_1+14*y_id[1] : x_2+14*y_id[1] ],
                                               student_att[i][ x_1+14*y_id[2] : x_2+14*y_id[2] ],
                                               student_att[i][ x_1+14*y_id[3] : x_2+14*y_id[3] ],
                                               student_att[i][ x_1+14*y_id[4] : x_2+14*y_id[4] ],
                                               student_att[i][ x_1+14*y_id[5] : x_2+14*y_id[5] ],
                                               student_att[i][ x_1+14*y_id[6] : x_2+14*y_id[6] ],
                                               student_att[i][ x_1+14*y_id[7] : x_2+14*y_id[7] ],
                                               student_att[i][ x_1+14*y_id[8] : x_2+14*y_id[8] ],
                                               student_att[i][ x_1+14*y_id[9] : x_2+14*y_id[9] ],
                                               student_att[i][ x_1+14*y_id[10] : x_2+14*y_id[10] ]
                                              ))
            teacher_att_top_11[i] = torch.cat((teacher_att[i][ x_1+14*y_id[0] : x_2+14*y_id[0] ],
                                               teacher_att[i][ x_1+14*y_id[1] : x_2+14*y_id[1] ],
                                               teacher_att[i][ x_1+14*y_id[2] : x_2+14*y_id[2] ],
                                               teacher_att[i][ x_1+14*y_id[3] : x_2+14*y_id[3] ],
                                               teacher_att[i][ x_1+14*y_id[4] : x_2+14*y_id[4] ],
                                               teacher_att[i][ x_1+14*y_id[5] : x_2+14*y_id[5] ],
                                               teacher_att[i][ x_1+14*y_id[6] : x_2+14*y_id[6] ],
                                               teacher_att[i][ x_1+14*y_id[7] : x_2+14*y_id[7] ],
                                               teacher_att[i][ x_1+14*y_id[8] : x_2+14*y_id[8] ],
                                               teacher_att[i][ x_1+14*y_id[9] : x_2+14*y_id[9] ],
                                               teacher_att[i][ x_1+14*y_id[10] : x_2+14*y_id[10] ]
                                              ))
            # 7*7 -----------------------------------------------------------------------------------
            y_id, x_1, x_2 = teacher_att_top_index[i][1]
            student_att_top_7[i] = torch.cat((student_att[i][ x_1+14*y_id[0] : x_2+14*y_id[0] ],
                                              student_att[i][ x_1+14*y_id[1] : x_2+14*y_id[1] ],
                                              student_att[i][ x_1+14*y_id[2] : x_2+14*y_id[2] ],
                                              student_att[i][ x_1+14*y_id[3] : x_2+14*y_id[3] ],
                                              student_att[i][ x_1+14*y_id[4] : x_2+14*y_id[4] ],
            ...
            teacher_att_top_7[i] = torch.cat((teacher_att[i][ x_1+14*y_id[0] : x_2+14*y_id[0] ],
                                              teacher_att[i][ x_1+14*y_id[1] : x_2+14*y_id[1] ],
                                              teacher_att[i][ x_1+14*y_id[2] : x_2+14*y_id[2] ],
                                              teacher_att[i][ x_1+14*y_id[3] : x_2+14*y_id[3] ],
                                              teacher_att[i][ x_1+14*y_id[4] : x_2+14*y_id[4] ],
            ...
            # 3*3 -----------------------------------------------------------------------------------
            y_id, x_1, x_2 = teacher_att_top_index[i][2]
            student_att_top_3[i] = torch.cat((student_att[i][ x_1+14*y_id[0] : x_2+14*y_id[0] ],
                                              student_att[i][ x_1+14*y_id[1] : x_2+14*y_id[1] ],
                                              student_att[i][ x_1+14*y_id[2] : x_2+14*y_id[2] ],
                                             ))
            teacher_att_top_3[i] = torch.cat((teacher_att[i][ x_1+14*y_id[0] : x_2+14*y_id[0] ],
                                              teacher_att[i][ x_1+14*y_id[1] : x_2+14*y_id[1] ],
                                              teacher_att[i][ x_1+14*y_id[2] : x_2+14*y_id[2] ],
                                             ))
            
        student_att_top_11 = F.normalize(student_att_top_11)
        teacher_att_top_11 = F.normalize(teacher_att_top_11)
        student_att_top_7  = F.normalize(student_att_top_7)
        teacher_att_top_7  = F.normalize(teacher_att_top_7)
        ...
        if   self.att_loss_positive == 1:
            att_top_11_loss_per_sample = (student_att_top_11 - teacher_att_top_11).pow(2).mean(1)
            att_top_7_loss_per_sample  = (student_att_top_7 - teacher_att_top_7).pow(2).mean(1)
            att_top_3_loss_per_sample  = (student_att_top_3 - teacher_att_top_3).pow(2).mean(1)
        elif self.att_loss_positive == 0:
            att_top_11_loss_per_sample = (student_att_top_11*teacher_att_top_11).sum(1)
            att_top_7_loss_per_sample  = (student_att_top_7*teacher_att_top_7).sum(1)
        ...
        att_top_11_loss = self.gate.f(att_top_11_loss_per_sample, log, **kwargs)
        att_top_7_loss  = self.gate.f(att_top_7_loss_per_sample,  log, **kwargs)
        att_top_3_loss  = self.gate.f(att_top_3_loss_per_sample,  log, **kwargs)
          
        # ------------------------------------------------------------------------------------------------
        loss = self.loss_weight * ( (att_top_11_loss + att_top_7_loss + att_top_3_loss)/3 )
...
class KL_AttLoss(_LossBase):
    ...
    def forward(self, target_output, source_output, label_id, log=None, **kwargs):
        ...
        att_loss = (att_top_11_loss + att_top_7_loss + att_top_3_loss)/3
